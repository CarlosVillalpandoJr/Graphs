...
class Graph:
    ...
    def add_vertex(self, vertex_id):
        ...
        """
        self.vertices[vertex_id] = set() # set will represent edges
    ...
    def add_edge(self, v1, v2):
        ...
        if v1 in self.vertices and v2 in self.vertices:
            self.vertices[v1].add(v2) # Update set to add edge
        else:
    ...
    def dfs_recursive(self, starting_vertex, destination_vertex, visited = None, path = None):
        ...
        # Track visited nodes
        visited.add(starting_vertex)
        # cannot use a path because it would change reference 
        new_path = path + [starting_vertex]

        # DO THE THING! (this is a base case as well)
        if starting_vertex == destination_vertex:
            return path
        # Call the function recursively - on neighbors not visited 
        for neighbor in self.vertices[starting_vertex]:
            if neighbor not in visited:
                neighbor_path = self.dfs_recursive(neighbor, visited, destination_vertex, new_path)
                if neighbor_path:
